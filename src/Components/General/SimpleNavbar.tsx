...
const SimpleNavbar = () => {
    ...
    const viteSvg = (
        <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" aria-hidden="true" role="img" className="iconify iconify--logos" width="31.88" height="32" preserveAspectRatio="xMidYMid meet" viewBox="0 0 256 257">
            <defs>
    ...
    return(
        <nav className="bg-gray-800 text-white py-3 px-4 flex items-center justify-between">
            <div className="flex items-center select-none">
                <NavLink to='/'> {viteSvg} </NavLink>
            </div>
            <div className="flex items-center">
                { jwt && (<input 
                    type="text" onKeyDown={onSearchKeyDown} 
                    className="w-40 py-1 px-2 text-gray-700 rounded-lg border-2 border-gray-300 
                        focus:outline-none focus:border-blue-500" placeholder="Search..."/>) }
                <NavLink to={'/profile'} className={navbarButtonTailwindStyle}>
                    Profile
                </NavLink>
                <NavLink to='/chats' className={navbarButtonTailwindStyle}>
                    Chats
                </NavLink>
                {
                    jwt
